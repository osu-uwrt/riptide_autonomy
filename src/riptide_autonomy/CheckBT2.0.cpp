...
std::pair<bool,bool> findNodeInManifest(XMLElement *test, std::shared_ptr<BehaviorTreeFactory> factory){
    ...
    try{
        ...
        RCLCPP_WARN(log,"Riptide_autonomy does not have a valid c++ file for %s. Would you like to remove it from the workspace? (Y/n): ", test->Attribute("name"));
        std::string ans;
        std::cin>>ans;

        if(std::tolower(ans[0]) == 'y'){
            test->Parent()->DeleteChild(test);
            node_error = !tree.SaveFile(treePath.c_str()) == XML_SUCCESS;
        }
...
int main(int argc, char **argv) {
    ...
    if(argc = 1){
        ...
        //TODO: check the tree, different than checking workspace
    }
    else{
