...
class CallSetBoolService : public UWRTActionNode {
    ...
    BT::NodeStatus onStart() override {
        ...
        if(!client->wait_for_service(1s)) {
            RCLCPP_ERROR(log, "SetBool Service %s is not available.", srvName.c_str());
    ...
    BT::NodeStatus onRunning() override {
        ...
        if(!result.valid()) {
            RCLCPP_ERROR(log, "Result of SetBool service call to %s is invalid!", srvName.c_str());
            return BT::NodeStatus::FAILURE;
        }
        ...
        if((rosnode->get_clock()->now() - startTime).seconds() > timeoutSecs) {
            RCLCPP_ERROR(log, "Service call to %s took to long to respond.", srvName.c_str());
            return BT::NodeStatus::FAILURE;
        }
