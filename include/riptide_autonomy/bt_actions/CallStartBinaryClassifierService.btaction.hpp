...
class CallStartBinaryClassifierService : public UWRTActionNode {
    using StartBinaryClassifier = riptide_msgs2::srv::StartBinaryClassifier;

    public:
    CallStartBinaryClassifierService(const std::string& name, const BT::NodeConfiguration& config)
    : UWRTActionNode(name, config), 
      result(std::future<std::shared_ptr<StartBinaryClassifier::Response>>(), 0) { }
    ...
    BT::NodeStatus onStart() override {
        ...
        client = rosnode->create_client<StartBinaryClassifier>(srvName);
        
        //wait for client
        if(!client->wait_for_service(1s)) {
            RCLCPP_ERROR(rosNode()->get_logger(), "StartBinaryClassifier Service %s is not available.", srvName.c_str());
    ...
    BT::NodeStatus onRunning() override {
        ...
        if(!result.valid()) {
            RCLCPP_ERROR(rosNode()->get_logger(), "Result of StartBinaryClassifier service call to %s is invalid!", srvName.c_str());
            return BT::NodeStatus::FAILURE;
        }
        ...
        if(result.wait_for(0s) == std::future_status::ready) {
            ...
            StartBinaryClassifier::Response::SharedPtr resp = result.get(); 
    ...
    private:
    std::string srvName;
    rclcpp::Client<StartBinaryClassifier>::SharedPtr client;
    rclcpp::Client<StartBinaryClassifier>::FutureAndRequestId result;
